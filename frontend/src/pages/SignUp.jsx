...
export default function SignUp() {
  ...
  useEffect(() => {
    if (token) navigate('/games', { replace: true });
  }, [token, navigate]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      login(res.data.token, username, 'player');
      navigate('/games', { replace: true });
    } catch (err) {
      if (err.response?.data?.violations) {
        const v = {};
        for (const [key, val] of Object.entries(err.response.data.violations)) { v[key] = val.message; }
        setErrors(v);
      } else {
    ...
    <div className="min-h-screen flex items-center justify-center w-full p-5 bg-gradient-to-br from-teal-50 via-cyan-50 to-sky-50 relative overflow-hidden">
      ...
      <div className="w-full max-w-[440px] animate-in relative z-10">
        <div className="text-center mb-6">
          ...
          <p className="text-slate-400 text-sm mt-1">Join the gaming community</p>
        </div>
        ...
        <div className="bg-white/70 backdrop-blur-2xl border border-white/50 rounded-3xl shadow-xl overflow-hidden relative">
          ...
          <div className="p-6 md:p-10">
            <div className="text-center pb-2">
              <h3 className="text-2xl font-extrabold text-slate-900 mb-1 tracking-tight" style={{ fontFamily: "'Plus Jakarta Sans', 'Inter', sans-serif" }}>Create Account</h3>
              <p className="text-slate-400 text-sm">Start your gaming adventure today</p>
            </div>
            ...
            <form onSubmit={handleSubmit}>
              <div className="mb-4">
                ...
                <input className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:border-teal-600 focus:ring-3 focus:ring-teal-600/10 focus:outline-none transition-all placeholder:text-slate-400"
                  ...
                  placeholder="Choose a username (min 4 characters)" autoComplete="username"
                />
              ...
              <div className="mb-5 relative">
                ...
                <div className="relative">
                  <input className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:border-teal-600 focus:ring-3 focus:ring-teal-600/10 focus:outline-none transition-all placeholder:text-slate-400 pr-12"
                    type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} required disabled={loading}
                    placeholder="Create a password (5-10 characters)" autoComplete="new-password"
                  />
                  <button 
                    type="button" 
              ...
              <div className="flex flex-col gap-3 mt-4">
                <button type="submit" className="btn-premium w-full py-3 text-base" disabled={loading}>
                  {loading ? (
                    <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2"></span>Creating Account...</>
                  ) : 'Sign Up'}
                </button>
                <Link replace to="/" className="btn-ghost w-full py-3 text-center text-sm">
                  Back to Sign In
                </Link>
              </div>
            </form>
