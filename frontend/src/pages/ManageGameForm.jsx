...
export default function ManageGameForm() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await api.post('/games', { title, description });
      ...
      if (err.response?.data?.violations) {
        ...
        const errorMsg = typeof err.response?.data === 'string' ? err.response.data : (err.response?.data?.slug || err.response?.data?.message || 'Error creating game');
          ...
          <div className="bg-white/70 backdrop-blur-2xl border border-white/50 rounded-3xl shadow-xl overflow-hidden relative mt-5">
            ...
            <div className="p-6 md:p-10">
              <div className="text-center pb-2">
                <h3 className="text-2xl font-extrabold text-slate-900 mb-1 tracking-tight" style={{ fontFamily: "'Plus Jakarta Sans', 'Inter', sans-serif" }}>Create Game</h3>
                <p className="text-slate-400 text-sm">Publish your game to the portal</p>
              </div>
              ...
              <form onSubmit={handleSubmit}>
                <div className="mb-4">
                  ...
                  <input className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:border-teal-600 focus:ring-3 focus:ring-teal-600/10 focus:outline-none transition-all placeholder:text-slate-400"
                    value={title} onChange={e => setTitle(e.target.value)} disabled={loading} placeholder="Game title (min 3, max 60 chars)"
                  />
                  {errors.title && <small className="block mt-2 text-red-500 font-semibold text-xs">{errors.title}</small>}
                  {errors.slug && <small className="block mt-2 text-red-500 font-semibold text-xs">{errors.slug}</small>}
                </div>
                <div className="mb-4">
                  <label className="block text-slate-600 text-xs font-semibold uppercase tracking-wider mb-1.5">Description</label>
                  <textarea className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-3 text-sm focus:border-teal-600 focus:ring-3 focus:ring-teal-600/10 focus:outline-none transition-all placeholder:text-slate-400 resize-y"
                    rows={3} value={description} onChange={e => setDescription(e.target.value)} disabled={loading} placeholder="Describe your game (max 200 chars)"
                  />
                ...
                <div className="mb-4">
                  ...
                  <input className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-2.5 text-sm file:bg-gradient-to-br file:from-teal-600 file:to-emerald-600 file:text-white file:border-none file:rounded-lg file:px-4 file:py-1.5 file:font-semibold file:text-xs file:mr-3 file:cursor-pointer"
                    type="file" accept="image/png, image/jpeg, image/jpg" onChange={e => setThumbnailFile(e.target.files[0])} disabled={loading}
                  />
                  <small className="block mt-1 text-slate-400 text-xs">Optional: Select custom thumbnail to override game file</small>
                </div>
                <div className="mb-5">
                  <label className="block text-slate-600 text-xs font-semibold uppercase tracking-wider mb-1.5">Game Files (ZIP)</label>
                  <input className="w-full bg-white border-[1.5px] border-slate-200 text-slate-900 rounded-xl px-4 py-2.5 text-sm file:bg-gradient-to-br file:from-teal-600 file:to-emerald-600 file:text-white file:border-none file:rounded-lg file:px-4 file:py-1.5 file:font-semibold file:text-xs file:mr-3 file:cursor-pointer"
                    type="file" accept=".zip" onChange={e => setZipFile(e.target.files[0])} disabled={loading}
                  />
                  <small className="block mt-1 text-slate-400 text-xs">Upload a ZIP file containing your game (with index.html)</small>
                </div>
                <div className="flex gap-3">
                  <button type="submit" className="btn-premium flex-1 py-3" disabled={loading}>
                    {loading ? (<><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2"></span>Creating...</>) : 'Create Game'}
                  </button>
                  <Link to="/manage" className={`btn-ghost py-3 px-5 ${loading ? 'pointer-events-none' : ''}`}>Back</Link>
                </div>
              </form>
            </div>
          </div>
